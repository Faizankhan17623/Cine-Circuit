...
import { UserLogin, findloginemail } from '../../Services/operations/Auth'
...
const User = () => {
  ...
  useEffect(() => {
    ...
    const handler = setTimeout(async () => {
      ...
      try {
        const response = await dispatch(findloginemail(username))
        // console.log(response)
        if (response?.exists) {
          // Email found in DB — user exists, good for login
          setNames("found")
        } else {
